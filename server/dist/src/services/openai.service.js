...
// ----------------------------------------------------------------------------
function generateMockCourse(prompt, language) {
...
function buildMockLessons(topic, language) {
    const lessonTitles = language === "sw"
    ...
    return lessonTitles.map((title) => ({
        ...
        quiz: [
            {
                question: language === "sw" ? `Lengo kuu la ${title} ni nini?` : `What is the main goal of ${title}?`,
                options: language === "sw"
