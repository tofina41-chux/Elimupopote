...
export async function getOverview(req: Request, res: Response) {
  ...
  const [totalLearners, totalCourses, progressRows] = await Promise.all([
    ...
    prisma.progress.findMany({ where: { tenantId }, select: { completed: true } }),
  ]);
...
// ============================================================================
export async function getAtRiskLearners(req: Request, res: Response) {
  const tenantId = req.tenantId!;

  // (a) Learners whose most recent progress update is older than 7 days.
  const staleLearners = await prisma.$queryRaw<
    { id: string; fullName: string; phone: string; lastActivity: Date }[]
  >`
...
// ============================================================================
export async function sendAtRiskAlert(req: Request, res: Response) {
